...
Image reading_and_writing::Reader::Read() {
    ...
    if (!img.is_open()) {
        throw std::invalid_argument(std::string("Не удается открыть файл") + path_);
    }
    if (errno == EACCES) {
        throw std::invalid_argument(std::string("Доступ запрещен") + path_);
    }
    ...
    if (bmp_header[0] != image::utils::BMP_SIGNATURE[0] || bmp_header[1] != image::utils::BMP_SIGNATURE[1]) {
        throw std::invalid_argument(std::string("Неверный формат файла. Используйте BMP файлы") + path_);
    }
    try {
        ...
        throw std::invalid_argument("Не удается прочесть файл " + path_ + "\n" + static_cast<std::string>(e.what()));
    }
}
