...
std::unique_ptr<filters::Filter> filters::CreateFilter(const parse::Token& token) {
    ...
    if (filter == "-sharp") {
        if (!token.args.empty()) {
            throw std::invalid_argument("Sharpening фильтр не должен принимать аргументы");
        }
        return std::make_unique<filters::Sharpening>();
    } else if (filter == "-crop") {
        if (token.args.size() != 2) {
            throw std::invalid_argument("В Crop фильтр нужно передать 2 аргумента");
        }
        try {
            ...
            throw std::invalid_argument("Неправильный ввод данных для Сrop");
        }
    } else if (filter == "-gs") {
        if (!token.args.empty()) {
            throw std::invalid_argument("Grayscale фильтр не должен принимать аргументы");
        }
        return std::make_unique<filters::Grayscale>();
    } else if (filter == "-neg") {
        if (!token.args.empty()) {
            throw std::invalid_argument("Negative фильтр не должен принимать аргументы");
        }
        ...
        try {
            ...
            throw std::invalid_argument("Неправильный ввод данных для edge detection filter");
        }
    } else {
        throw std::invalid_argument("Неизвестное имя фильтра" + filter);
    }
}
